...
const initialCameraPos = { x: 0, y: 0, z: isMobile ? 18 : 10 };
  ...
  columns.forEach((col) => {
    ...
    setInterval(() => {
      ...
      for (let i = 0; i < 70; i++) {
        const char =
          Math.random() > 0.92
            ? fuaxChars[Math.floor(Math.random() * fuaxChars.length)]
...
const cubeData = [
  {
    ...
    links: [{ t: "Prints", u: "https://fuax-shop.fourthwall.com" }],
  },
  {
    id: 1,
    name: "Music",
    theme: 0xff00ff,
    pos: isMobile ? [2, 4, 0] : [0, 2, 0],
    links: [{ t: "SoundCloud", u: "https://soundcloud.com/faux-fuax" }],
  },
  {
    id: 2,
    name: "Dev",
...
window.addEventListener("click", () => {
  ...
  if (hits.length > 0 && !active) {
    ...
    // CRITICAL FIX: Trigger link spawn
    spawnChildren(active);

    new TWEEN.Tween(camera.position)
      .to({ x: active.position.x, y: active.position.y, z: 5 }, 800)
...
function spawnChildren(parent) {
  parent.userData.links.forEach((link, i) => {
    ...
    child.position.set(
      parent.position.x + Math.cos(angle) * 2.5,
      parent.position.y + Math.sin(angle) * 2.5,
      parent.position.z,
    );
    child.add(
      new THREE.LineSegments(
        new THREE.EdgesGeometry(childGeo),
        new THREE.LineBasicMaterial({ color: parent.userData.theme }),
      ),
    );
    const label = createTextLabel(
      link.t,
      `#${parent.userData.theme.toString(16).padStart(6, "0")}`,
      true,
    );
    label.position.set(0, 1.2, 0);
    child.add(label);
    child.userData = { url: link.u };
    scene.add(child);
    childCubes.push(child);
  });
}
...
function showDossier() {
  ...
  const bio = `> DECRYPTING PERSONNEL HISTORY...\n> SUBJECT: REMINGTON B.\n\nBuilt during a survival transition. Self-taught HTML/CSS/JS. Now mastering Web3 and LLMs.\n\n> STATUS: FOREVER UNFINISHED.`;
  let i = 0;
  textField.innerHTML = "";
  const typing = setInterval(() => {
    if (i < bio.length) {
      textField.innerHTML += bio.charAt(i) === "\n" ? "<br>" : bio.charAt(i);
      i++;
    } else {
  ...
  if (neuralLinkEstablished) {
    ...
    if (!active) {
      ...
      if (hits.length > 0) {
        ...
        if (hovered !== obj) {
          if (hovered) {
            const oldLine = hovered.children.find(
              (c) => c.type === "LineSegments",
            );
            if (oldLine) oldLine.material.color.setHex(0x00fff7);
            hovered.getObjectByName("hoverLabel").material.opacity = 0;
          }
          hovered = obj;
          sfx.hover.play().catch(() => {});
          const lbl = hovered.getObjectByName("hoverLabel");
          lbl.material.opacity = 1;
          lbl.material.color.setHex(hovered.userData.theme);
        }
        const currentLine = hovered.children.find(
          (c) => c.type === "LineSegments",
        );
        if (currentLine)
          currentLine.material.color.setHex(
            Math.random() > 0.85 ? 0x008888 : hovered.userData.theme,
          );
        document.getElementById("instructions").innerText =
          `[ ${hovered.userData.name.toUpperCase()} ]`;
      } else if (hovered) {
        const oldLine = hovered.children.find((c) => c.type === "LineSegments");
        if (oldLine) oldLine.material.color.setHex(0x00fff7);
        hovered.getObjectByName("hoverLabel").material.opacity = 0;
        hovered = null;
        document.getElementById("instructions").innerText =
